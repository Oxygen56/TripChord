...
| C-122 最终 HEAD 受控重启 | `/api/v1/agents/runtime` provenance（round-18 最终 HEAD 三哈希匹配、pid 存活；机器证据在 gate run 发布的 side-channel evidence 与最终结果评论） | API 已绑定**round-18 最终 HEAD**（代码+文档全提交后受控重启；取代 C-118 的 `048ba57` 绑定，后者为中间态不再构成绑定） | 只证明运行代码=最终 HEAD；不证明六层门通过 |
| C-122 六层门重跑 | side-channel `refs/tripchord/done-gate/<run_id>`（P→E→S + manifest blob 验签） | `TRIPCHORD_ACK_MODEL_COST=1` 授权下从零跑严格六层门，绑定 round-18 最终 HEAD；`passed=false`、evidence_commit 与工作树状态如实记录在最终结果评论 | 未过真实门不宣称通过；五浏览器 scope 需用户配对 Companion 并保持官方域名登录态 |
