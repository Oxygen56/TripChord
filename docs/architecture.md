...
本文描述当前 2.0 的统一实现。单出发地、单目的地往返是保留的快速路径；多城市、异地进出、分组同行、固定活动、按需 Agent 和外部 MCP 接入都通过同一领域合同进入。更完整的领域模型与迁移取舍见[复杂自由行目标架构](target-architecture.md)。
...
这些角色不是按日期复制的聊天机器人，而是共享同一类型化运行时、使用不同输入合同、上下文预算、工具白名单和输出结构的专业决策节点。2.0 通过确定性 Need Router 只在歧义、冲突、取舍或特殊风险出现时触发相应角色；Query Strategist、Event Diagnoser 等职责属于特定路径。下面列的是职责边界，不是每次运行的模型调用清单。
...
2.0 已把自然语言需求、灵活日期、多城市、分组同行、只读来源、同行价格、组件组合、活动、按需 Agent、版本化修改和人民币方案接入同一主入口。它的验收依据是保存来源回放与范围受控当前来源组成的端到端链，而不是 Agent 数量或版本名称。
