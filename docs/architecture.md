...
长任务的状态、幂等键、租约、取消意图和逐日期结果已经进入 PostgreSQL 控制面；下一步是把浏览器采集、短期报价复用和运行结果投影纳入同一恢复边界。只有某个模型子流程真实出现跨重启暂停恢复、人工介入或复杂循环，并且隔离试验能证明收益时，才局部引入 LangGraph。若未来需要跨机器、小时级可靠执行，再评估 Temporal 作为外层持久工作流。
