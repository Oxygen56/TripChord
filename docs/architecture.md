...
TripChord 要解决的不是“生成一段旅行建议”，而是把一个带有灵活日期、同行者、固定安排和个性化偏好的旅行需求，转化为一份交通、住宿、接驳与活动彼此成立、总价口径一致、可以直接帮助用户做决定的出行方案。
...
当前产品版本为 **2.0.0**。这里的 2.0 表示七轮范围受控功能形态已经沿统一旅行状态、报价合同、求解器和行程卡链路闭合；它不代表签名安装包、生产稳定性、长期真实平台成功率或商业采用已经完成。逐轮结果与边界见 [2.0 公共验收](v2.0-acceptance.md)，单目的地历史基线见 [1.0 桌面核心验收](v1.0-acceptance.md)。

本文描述当前 2.0 的统一实现。单出发地、单目的地往返是保留的快速路径；多城市、异地进出、分组同行、固定活动、复杂候选上的三角色 Multi-Agent 面板和外部 MCP 接入都通过同一领域合同进入。更完整的领域模型与迁移取舍见[复杂自由行目标架构](target-architecture.md)。
...
当用户偏好明确且查询结果足以支持可靠选择时，2.0 给出一份最终出行方案（代码中的 `FinalPlan`）；偏好不足时可从同一批候选给出代表方案。方案包含：
...
系统先排除日期、人数、住宿和衔接不成立的方案。对本次确实查到、产品能够对上且费用口径一致的可行方案，先满足用户不能妥协的要求，再按当前请求的价格、时间和体验取舍选择；没有足够取舍信号时，从同一批候选生成省钱、均衡和体验优先代表方案。

这个选择规则及其算法边界见 [D002：用户目标决定什么叫最佳方案](architecture-decisions.md#d002用户目标决定什么叫最佳方案)。
...
内部可以保留多个候选和淘汰原因，诊断 API 也可以返回它们。2.0 的产品界面在偏好明确时展示一份收敛后的方案；偏好不足时从同一批候选展示省钱、均衡和体验优先代表方案，而不是把全部候选重新交给用户筛选。
...
这些角色不是按日期复制的聊天机器人，而是共享同一类型化运行时、使用不同输入合同、上下文预算、工具白名单和输出结构的专业决策节点。当前复杂候选路径固定使用价格 Agent、体验 Agent和综合决策 Agent：前两个并行，第三个接收结构化提案；唯一候选时不启动无意义的面板。Query Strategist、Event Diagnoser 等职责属于其他特定路径。下面列的是职责边界，不是按日期或平台复制的模型实例。
...
这些角色共享同一类型化运行时、模型路由和进程，因此逻辑隔离不等于物理隔离，仍存在共同故障域。面板的价值来自价格视角、体验视角和综合裁决之间的上下文隔离与结构化交接，而不是角色数量。每个角色的提案必须绑定同一 `graph_version`、候选 ID 和来源引用，最终候选仍由确定性程序复算。
...
当前已有一个最小外部 Agent 接入层：stdio MCP 适配器只暴露创建、查状态、读方案和修改四个完整任务能力，外部宿主不能绕过 TripRun、来源核对或最终发布检查。新的旅行能力仍应优先实现为带输入、输出、权限、错误说明和测试的平台连接模块或领域工具；不把内部角色逐个包装成工具。
...
2.0 已把自然语言需求、灵活日期、多城市、分组同行、只读来源、同行价格、组件组合、活动、三角色 Multi-Agent 取舍面板、版本化修改和人民币方案接入同一主入口。它的验收依据是保存来源回放与范围受控当前来源组成的端到端链，而不是 Agent 数量或版本名称。

2.0 之后才继续手机端、减少或无头化浏览器标签、更多平台与目的地、签名安装升级，以及持续真实运行的成功率与性能验证。这些成熟化工作不能被描述成 2.0 已完成能力。

当前状态与逐轮证据见 [TripChord 2.0 公共验收](v2.0-acceptance.md)；后续成熟化顺序见 [TripChord 产品形态与实施路线](roadmap.md)。
...
- [2.0 七轮公共验收](v2.0-acceptance.md)
- [1.0 桌面核心历史基线](v1.0-acceptance.md)
