...
> 状态：目标与主架构已确认，尚未全部实现。当前可运行能力仍以 [1.0 验收记录](v1.0-acceptance.md) 为准。
...
> 类型化旅行问题模型 + 三种旅行图视图 + 按问题形态选择的确定性求解器 + 按需出现的专业 Agent。
...
flowchart TD
  ...
  S2 --> F
  S3 --> F
  F --> N{Agent Need Router}
  N -->|无需语义判断| V[程序独立复算]
  N -->|存在歧义、冲突、取舍或特殊风险| G[相关专业 Agent]
  G --> V
...
| 固定运行完整 Multi-Agent 团队 | 日期、平台和候选规模并不需要更多模型；固定调用只会放大耗时、上下文和错误传播 |
...
Multi-Agent 不是用角色数量表达复杂度，而是用**不同语义上下文和独立判断职责**解决程序无法可靠写死的问题。一个确定性的 `Agent Need Router` 根据当前问题决定是否调用角色。
...
| 生成可行方案 | 求解器组合 | 不调用模型 |
| 从 Pareto 候选中做体验取舍 | Decision Agent | 确实存在多个非支配候选且用户偏好需要解释时 |
...
### 阶段四：让 Agent、上下文、记忆和 Skill 按需工作

实现确定性 Need Router；将来源任务、日期任务、金额核对和最终验证从“Agent 角色”中剥离。只在歧义、冲突、取舍、特殊风险或显式记忆信号出现时调用对应角色，并给它最小相关子图与版本化 Skill。
...
| Agent 价值对照 | 简单任务跳过模型仍得同一结果；有真实取舍时按需 Agent 优于单模型与固定团队 | Multi-Agent 的必要性来自任务，不来自包装 |
| 性能基线 | 新合同和求解不会让当前保存数据流程出现明显回退；查询仍是主要耗时或有明确数据解释变化 | 架构没有制造新的主瓶颈 |

结构验证不等于真实平台覆盖。阶段一至四证明的是领域表达、求解和 Agent 边界；阶段五才增加一份有限的真实扩展证据。任何公开说明都必须区分二者。
...
- 当前 Runtime 直接阻碍按需 Agent、任务恢复或单一状态，而且局部替换无法解决；
- 来源长期只提供不可拆整包，使原子报价假设不成立；
- 真实盲测显示按需 Multi-Agent 没有改善取舍质量，或增加的错误与耗时大于收益。
