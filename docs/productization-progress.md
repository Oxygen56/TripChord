...
- **当前分支**：`productization/v1.0`（未 push；**R43 文档收尾为本 docs-only 提交，产生新 HEAD（当前提交），已越过 R42 实现基线 `dc075ef`**。API 与旧六层门 `f83b` 证据只绑定 **R42 已测试实现基线 `dc075ef`**（provenance 三哈希匹配、`passed=true`、pid 与三哈希见 provenance 机器证据与最终结果评论、mismatches 空），**不绑定 R43 文档新 HEAD**。待用户 Companion 授权后必须先受控重启 API 绑定 R43 新 HEAD，再从头重跑六层门，证据 `tested_commit` 与新 HEAD 一致后才可终验。）
- **基线 commit**：`0fa8f78`（chore: baseline productization contract and roadmap）
- **工作目录**：`/Users/oxygen/Documents/个人项目/tripchord`
- **最后完成的最小任务**：C-122 R42 Block 90（监督打回十一）——`_json_member_key_at` 真实 quoted span 成员键解析修复首字符转义 documented 键误拒；生成式矩阵 144 main + 576 ws{SPACE,TAB,CR,LF} 全唯一 + 32 plan{planner/provider_version} 子键 = **752 例全五路正例 + 24 负例**全通过（聚焦 10 passed、全量 gate 491 collected、全量 1526 passed、clean-env 1526 passed、ruff 全绿）。**该项为 R42 已测试实现/旧证据基线 `dc075ef`**；R43 文档收尾（本提交）仅改 docs，未触碰代码/测试/gate/账本。
...
- **审查/终验状态**：C-125 独立架构师审查对 **R42 Block90 已测试实现基线 `dc075ef`** **PASS**（仅 Block90 代码审查范围，无新增绕过/删测/gate 放松）；C-124 终验尚未启动（保持 backlog）。C-122 保持 in_progress，直至 API 绑定 R43 新 HEAD、同新 HEAD 六层 `passed=true` 且证据 `tested_commit` 与新 HEAD 一致后最终收口。
...
### 第四十四轮（C-122 R42 Block 89 监督打回十）验证结果

> 监督打回十：Block88 让 documented 外层真实数组元素在**携带数组路径**上位置无关豁免后，又发现两个**合法 JSON 形式被误拒**的五路不对称：①**转义 documented 成员键**——RFC-8259 允许 JSON 成员键带转义序列（`{"planner_version":["plannerV2","x"]}` 中 `_` 即 `_`），raw narration 回退从 **RAW 键文本**建携带路径，解码后的键从不匹配 documented registry → 误拒；②**数组括号间合法 JSON 空白**——SPACE/TAB/CR/LF 均属 JSON 空白（`{"planner_version":[ [ "plannerV2" ]] }`），exact-base 赋值 regex 捕获 malformed `[ [ "plannerV2" ]] }`（`_WRAPPED_BASE_ILLEGAL`——数组 `[` 未在 token 内闭合、尾 `}` 跨匹配），Block88 helper 只跳 `[` + 一个空格后再见 `[` 而非 `"` → 判假误拒。两者均为 raw committed/failure 双 final 拒绝、producer 不动、consumer 接受。修复要求：①escaped documented 键、SPACE/TAB/CR/LF 括号空白按既有 documented 契约接受（first/middle/last/nested 一套契约）；②五路矩阵——真实 JSON 层 + depth 2-4 的 escaped-key/空白形式（raw committed/failure 双 final→producer→真实 0600 seal→consumer→双 final 全链），证明 producer 不过度遮盖、双 final 一致接受；③保留 escaped cross-field/unbound/dotted/真实 `'[]'` key/duplicate 负例及 Block87/88 全回归；④不回归 + 重跑全套（聚焦/全量 gate/apps-api+benchmarks 全量/clean-env/ruff）→提交干净 HEAD→API 绑定最终 HEAD、provenance passed=true→同 HEAD 从头六层门；`passed=false` 不得收口，C-122 保持 in_progress，C-125/C-124 保持 backlog 禁止提前审查/终验；一次性收口，之后不再新增提交。

修复：`_documented_version_field_exempt` 与 `_is_documented_json_array_element_assign` 在 `is_json_document` 时用 `_decode_json_string_escapes` 对成员键解码（与 JSON walker 看到的一致），自由文本赋值（非 JSON）永不解码——prose 里字面 `_` 仍按原键 fail-closed；数组 opener 循环反复跳过 `[` + 全部 JSON 空白（` \t\r\n`）后再要求双引号字符串元素。Block88 的携带数组路径豁免、Block87 的 typed sentinel、Block86 per-item 判定、Block85 recursion-first、Block84 path-carrying 全部保持。

| 命令 | 结果 |
|---|---|
| `uv run pytest scripts/tests/test_run_product_done_gate.py -k "block82 or block83 or block84 or block85 or block86 or block87 or block88 or block89"`（聚焦） | 9 passed（Block89 新增 escaped documented 键 + 括号空白五路矩阵 + Block88/87 反例全保留 + Block82–88 不回归断言） |
| `uv run pytest scripts/tests/test_run_product_done_gate.py`（全量 gate 测试） | 490 collected，退出码 0 |
| `uv run pytest apps/api/tests benchmarks/tests scripts/tests`（全量） | 1525 passed，退出码 0 |
| `uv run python scripts/tests/run_tests_clean_env.py apps/api/tests benchmarks/tests scripts/tests`（clean-env） | 1523 passed + 2 个 OR-tools 规模基准（`benchmarks/tests/test_evaluate_scale.py::test_scaled_planning_and_ablation_benchmark`、`test_scaled_replanning_preserves_unaffected_items`）`PlanningInfeasible`（must-visit `s1-a0`）——load-flaky：isolated clean-env 复跑 2 passed（35.5s），与本次 `_secret_redact.py` redaction 改动无关（全量非 clean-env 下同两测试通过）；clean-env 重定向临时 DB/bridge-state，live `tripchord.db`/`.runtime/browser-bridge-state.json` 不被测试触碰 |
| `uv run ruff check .` | All checks passed |
| `git rev-parse HEAD` | 最终 HEAD（当前账本提交；`_secret_redact.py` + gate 测试 + 账本/进度文档同提交，精确 SHA 见最终结果评论） |
| API 重启 + `uv run python scripts/verify_api_runtime_provenance.py` | API 受控重启绑定最终 HEAD（本提交）；provenance `passed=true`、`commit_sha`/`dependency_lock_sha256`/`live_system_source_sha256` 三哈希匹配、pid 存活、mismatches 空（精确值见最终结果评论） |
...
### 第四十五轮（C-122 R42 Block 90 监督打回十一）验证结果

> 监督打回十一：Block89 让转义 documented 成员键按解码键豁免后，又发现**首字符转义**的成员键仍被误拒——free-text 赋值 regex 的 field 以 word 字符开头，`{"planner_version":["plannerV2","x"]}`（`p` 即 `p`）键首字符是反斜杠转义时，捕获从反斜杠**之后**开始（field=`u0070lanner_version"`），真实键首字符上下文丢失；Block89 只解码 regex 捕获的 field，截断 field 永远解不出 documented 键 → raw committed/failure 双 final 拒绝、producer 不动、consumer 接受——五路不对称。修复要求：必须从**真实已解析 JSON 的 quoted span/member path** 取 decoded key/typed value，不得继续依赖截断 regex field、全局替换或字符表修补；补齐 leading/full escaped planner/provider/summary/plan-inner、array+scalar、首/中/末/nested、depth 2–4 全链；保留 Block87–89 负例；重跑全套（聚焦/全量 gate/apps-api+benchmarks 全量/clean-env/ruff）→提交干净 HEAD→API 绑定最终 HEAD、provenance passed=true→同 HEAD 从头六层门；`passed=false` 不得收口，C-122 保持 in_progress，C-125/C-124 保持 backlog 禁止提前审查/终验；一次性收口，之后不再新增提交；交付前精确清理本轮临时 `.probe_b89.py`（确认归属后），不得提交或遗留。

修复：新增 `_json_member_key_at`——在真实已解析 JSON 的 quoted span 中定位包含 field 起点的字符串字面量，若闭引号后是 `:`（成员 KEY）则返回 RFC-8259 解码后的真实键（与 JSON walker 看到的一致）；value 位置 span（闭引号后非 `:`）是 prose 非成员路径、返回 None，走原 regex field 回退。`_documented_version_field_exempt` 与 `_is_documented_json_array_element_assign` 均改为从真实 span 取键，三处调用点传 text+spans；不再依赖截断 regex field、全局替换或字符表修补。Block88 携带数组路径豁免、Block87 typed sentinel、Block86 per-item 判定、Block85 recursion-first、Block84 path-carrying 全部保持。监督打回十一补充要求（matrix 增量复核）将正式矩阵升级为**可审计生成式覆盖**：测试函数按 Cartesian 维度代码枚举并做数量断言与唯一性断言（主矩阵 2 转义样式×3 documented 键×6 value 形式×depth 0/2/3/4 = **144**；空白矩阵 JSON 空白 {SPACE,TAB,CR,LF} 四型独立×2×3×6×4 = **576**——scalar 形式把实际空白注入 member colon/value 边界，`len(set(ws_pos)) == 576` 全部唯一；转义 `plan` 父×转义子（`planner_version`/`provider_version` 各 leading/full）×depth = **32**；负例 12+12；全部 `assert len(...)` 数量断言 + `assert len(set(...)) == <count>` 唯一性断言，缺任一维度或重复 padding 即测试失败），所有正例（含 depth 2/3/4 编码）均走完整真实五路（raw committed/failure→producer 不动→真实 0600 seal→consumer→双 final），共 **752 例全五路正例**。临时 `.probe_b89.py` 已确认归属并清理（git log 全历史无该文件提交，工作树无该文件）。

| 命令 | 结果 |
|---|---|
| `uv run pytest scripts/tests/test_run_product_done_gate.py -k "block82 or block83 or block84 or block85 or block86 or block87 or block88 or block89 or block90"`（聚焦） | 10 passed（Block90 可审计生成式矩阵：主矩阵 2×3×6×4=144 + 空白矩阵 SPACE/TAB/CR/LF 四型×2×3×6×4=576（scalar 实际注入空白、`len(set)==576` 全唯一）+ 转义 plan 父×`planner_version`/`provider_version` 子×4=32，**752 例全部走完整真实五路**，逐维度数量断言 + 唯一性断言；Block87–90 负例 12/12 保留 + Block82–89 不回归断言） |
| `uv run pytest scripts/tests/test_run_product_done_gate.py`（全量 gate 测试） | 491 collected，退出码 0 |
| `uv run pytest apps/api/tests benchmarks/tests scripts/tests`（全量） | 1526 passed，退出码 0 |
| `uv run python scripts/tests/run_tests_clean_env.py apps/api/tests benchmarks/tests scripts/tests`（clean-env） | 1526 passed，退出码 0（含 OR-tools 规模基准，本轮无 load-flaky） |
| `uv run ruff check .` | All checks passed |
| `git rev-parse HEAD` | 最终 HEAD（当前账本提交；`_secret_redact.py` + gate 测试 + 账本/进度文档同提交，精确 SHA 见最终结果评论） |
| API 重启 + `uv run python scripts/verify_api_runtime_provenance.py` | API 受控重启绑定最终 HEAD（本提交）；provenance `passed=true`、`commit_sha`/`dependency_lock_sha256`/`live_system_source_sha256` 三哈希匹配、pid 存活、mismatches 空（精确值见最终结果评论） |
| `TRIPCHORD_ACK_MODEL_COST=1 uv run python scripts/run_product_done_gate.py --commit <最终 HEAD 全 SHA> --commit-evidence` | 同 HEAD 从头重跑严格六层门（最新 evidence 目录，run_id 与 tested_commit_sha 见最终结果评论）：层 1/2/3/4 PASS（层 4 required-model smoke 实际运行通过）、层 5 FAIL（pending user authorization：非全部 certified canary scope 有 fresh authorised 只读 canary）、层 6 FAIL——executor 在 done-gate 前于 `companion_preflight` 失败（未发现同时声明携程/去哪儿/同程且仍新鲜的已连接 Companion，心跳过期 >45s，实时搜索未提交）；`passed=false` 退出码 2、`worktree_dirty=false`、evidence 未提交（evidence_commit=null）、gate_ref=null。`passed=false` 如实记录、不得收口：C-122 保持 in_progress，C-125/C-124 保持 backlog 不提前审查/终验 |
