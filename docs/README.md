...
4. [复杂自由行目标架构](target-architecture.md)：未来如何支持多城市、分组同行、活动、按需 Agent、记忆和组合求解；
5. [当前系统架构](architecture.md)：1.0 中模型角色和普通程序怎样分工；
6. [重要架构决策](architecture-decisions.md)：为什么这样设计，以及哪些方案已经比较后没有采用；
7. [产品形态与实施路线](roadmap.md)：完整产品要做到什么，下一步按什么顺序实施；
8. [1.0 桌面核心验收](v1.0-acceptance.md)：当前版本已经实际完成到哪里。
