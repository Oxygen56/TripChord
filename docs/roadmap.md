# TripChord 1.0 核心与 1.x 成熟化路线

当前产品版本为 **1.0.0**。1.0 表示桌面核心功能已经接成一条可交付主链；保存真实来源数据驱动的验收见 [1.0 桌面核心验收](v1.0-acceptance.md)。它不表示签名安装包、普通用户安装体验、持续真实平台成功率或生产稳定性已经完成。

本文保留原始完整产品蓝图，用来说明 1.x 如何继续成熟。凡是安装、手机、多标签或无头并行、更多平台、长期真实运行和正式支持流程，均是 1.0 之后的工作，不得倒写成当前已交付能力。

`1.0.0` 是当前功能合同。后续能力仍以用户可感知结果和真实运行数据验收，不另加会让完成状态含混的阶段标签。
...
TripChord 是一套**本地优先的旅行决策产品**，不是托管用户平台账号的云服务。下面的安装、升级和回滚条目属于 1.x 后续成熟化目标，不是当前 1.0.0 已完成声明：
...
- 后续成熟版本应提供安装、自动升级、失败回滚和卸载，不要求用户维护开发环境。
...
TripChord 保留当前十二种专业职责，但不以数量作为亮点。当前核心实时规划使用固定的审查与修复任务图，Query Strategist、Event Diagnoser 等职责属于特定路径；因此这张表是能力分工，不是每次运行的模型调用清单。1.x 会继续通过对照评测决定哪些职责保持常驻、延后执行或改为条件触发，并让每个需要调用模型的角色都有自己的业务知识、可用工具、可见信息和清楚输出，而不是只换一段提示词：
...
下面九个阶段是原始产品化蓝图。桌面核心一至七的主链已进入 1.0.0；阶段八、九及各阶段尚未满足的持续运行、安装和支持标准继续作为 1.x 后续增强目标，不能因版本号改变而视为完成。
...
| 八、持续真实运行 | 非开发者可以在公开支持范围内持续完成真实规划 |
| 九、Stable 发布 | 产品可以独立安装、升级、回滚、恢复和长期维护 |

### 当前基础并不是从零开始

| 方向 | 已经具备 | 距离成熟形态还缺什么 |
| --- | --- | --- |
| 产品与界面 | 自然语言需求、单一最终行程、人民币费用、任务状态、偏好与中文局部修改已经接入同一桌面主入口 | 签名安装封装、普通用户首次设置与持续可用性验证 |
| 长任务 | 规划任务、浏览器租约、取消、逐日期结果、短期报价复用和结果投影已进入 PostgreSQL 恢复边界 | 跨较长周期、多进程与真实平台异常的持续运行验证 |
...
| Multi-Agent | 类型化专业职责、上下文、工具白名单、条件模型调用和动态参与声明 | 继续以单模型、纯规则和条件多角色对照证明每个模型调用的边际价值 |
...
| 版本称呼 | 需要达到的状态 |
| --- | --- |
| `1.0.0`（当前） | 桌面核心主链有保存真实来源数据驱动的可复现验收；仍不宣称生产稳定、安装成熟或持续真实成功 |
| `1.x` | 扩大真实平台、目的地与同行人覆盖，并以明确分母持续记录成功率、耗时和失败 |
| `1.x / RC` | 签名安装、迁移、故障恢复、安全和回滚检查完成，准备稳定发布 |
| `1.x / Stable` | 在公开支持范围内完成预先登记的持续运行观察，没有未关闭的发布级问题 |
...
- 每次规划的模型调用、token、人民币成本、平台请求和本机资源都在真实运行前登记的预算内；超出预算时先优化，不能只证明质量而忽略使用成本。
...
- 真实运行报告清楚列出支持范围、成功率、p50/p95 耗时和仍不支持的情况；
- 成功运行必须来自新发起、已获授权的标准入口：本次选择的所有平台任务都有明确结果，交通和住宿达到当前精确报价要求，规划、程序检查、必要修复、修复后复查和最终关键价格刷新全部完成；缺一项只能明确停止，不能计入成功。
- 每次真实运行都能回到同一个代码提交、运行时版本、平台连接版本和依赖锁文件，避免拿不同版本的局部结果拼成一次成功。

### 阶段九：完成 RC 与 Stable 发布
...
- Stable 的性能、可靠性和支持范围经过持续运行验证，而不是由版本名称宣布；
- GitHub 的代码、发布物、文档和 CI 始终对应同一个版本；
- RC 和 Stable 的最终验收必须从候选安装包新发起一次已获授权的标准真实规划，并与本次发布提交、运行时、平台连接版本和依赖锁定信息一致；
- [仓库正式发布检查](done-gate.md) 必须以成功状态结束，机器报告和最终发布检查都明确通过；任一条件缺失都不能发布 RC 或 Stable。
...
- 如果单次规划的模型费用、平台请求或本机资源超出真实运行前登记的预算，就先优化调用和缓存，不把成本转嫁给用户后继续扩张。

## 6. 成熟发布的补充验收

桌面核心 1.0 的当前验收见独立事实包。下面保留的是 RC 与 Stable 所需的更高门槛；当前版本没有因为使用 `1.0.0` 而自动满足这些条目。
...
5. 减少桌面浏览器标签，评估经授权的无头并行；不得牺牲已有登录态、来源可见性和只读边界。
6. 在有明确收益时，为局部模型流程引入 LangGraph，或为跨机器长任务引入 Temporal。
7. 在用户明确同意、排序不受佣金影响且符合平台规则时，再评估 affiliate 模式。
