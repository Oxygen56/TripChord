...
当前产品版本为 **1.0.0 / Beta**。1.0 表示桌面核心功能已经接成一条可交付主链；保存真实来源数据驱动的验收见 [1.0 桌面核心验收](v1.0-acceptance.md)。它不表示签名安装包、普通用户安装体验、持续真实平台成功率或生产稳定性已经完成。

本文保留原始完整产品蓝图，用来说明 1.x 如何继续成熟。凡是安装、手机、多标签或无头并行、更多平台、长期真实运行和正式支持流程，均是 1.0 之后的工作，不得倒写成当前已交付能力。

版本号与成熟度是两条轴：`1.0.0` 是功能合同，Beta 是当前工程成熟度。后续仍以用户可感知结果和真实运行数据决定是否升级成熟度。
...
下面九个阶段是原始产品化蓝图。桌面核心一至七的主链已进入 1.0.0；阶段八、九及各阶段尚未满足的持续运行、安装和支持标准继续作为 1.x Beta 成熟化门槛，不能因版本号改变而视为完成。
...
| 版本称呼 | 需要达到的状态 |
| --- | --- |
| `1.0.0 / Beta`（当前） | 桌面核心主链有保存真实来源数据驱动的可复现验收；仍不宣称生产稳定、安装成熟或持续真实成功 |
| `1.x / Beta` | 扩大真实平台、目的地与同行人覆盖，并以明确分母持续记录成功率、耗时和失败 |
