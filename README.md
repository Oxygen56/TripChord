...
**2.0 的准确边界** 是范围受控的复杂自由行决策能力，不是对任意旅行需求的普遍保证。候选存在真实价格与体验取舍时，配置模型服务后会启动价格、体验和综合决策三个 Agent；偏好不足时仍从同一次查询得到的候选中提供省钱、均衡和体验优先三种代表方案。用户确认过的稳定偏好可以被记住，当前请求永远可以覆盖历史偏好。价格、库存和班次始终只来自本次查询。
...
面向复杂自由行的领域模型、组合算法、三角色 Multi-Agent 面板、上下文、记忆与渐进迁移方案，见[复杂自由行目标架构](docs/target-architecture.md)。
...
外部 Agent 接入已经有一条范围受控的 MCP 证据链：Claude Code 2.1.246 可创建、断开后恢复并修改同一个 TripRun，官方 Python MCP SDK 1.29.1 可独立读取相同状态。详见 [I7 MCP 当前来源证据](benchmarks/results/i7-mcp-current-2026-08-26.json)；这不等于已经覆盖所有宿主或完成全网旅行能力。
...
TripChord 不让多个模型自由聊天。每个角色只看到完成自己工作所需的信息，使用限定工具，并输出可以被程序检查的结构化结果。复杂候选目录会先由程序固定来源、日期、人数和价格口径，再并行调用价格 Agent 与体验 Agent，最后由综合决策 Agent 接收前两者提案；程序负责验证所有候选身份、金额和来源引用，并完成最终仲裁。只有没有可供语义取舍的唯一候选时，才不启动这一组模型。一次真实模型面板记录见 [Multi-Agent 当前证据](benchmarks/results/multi-agent-panel-current-2026-08-26.json)。

当前真实模型面板已经完成三次结构化调用：价格与体验 Agent 并行提案，综合决策 Agent 接收两份提案，程序再完成候选核验与仲裁；这条证据使用冻结报价目录，不代表实时 OTA 价格、全网最低价或下单成功。历史马尔代夫运行仅作为来源边界记录保留，详见[真实运行记录](docs/real-run-maldives-2026-08-24.md)和[公开声明与运行记录](docs/claim-ledger.md)。
...
| 方案选择 | 价格 Agent 与体验 Agent 分别提出候选，综合决策 Agent 接收提案并做有限取舍；最终候选仍由程序复核 |
...
| Multi-Agent 容易拖慢 | 价格与体验 Agent 并行，综合决策 Agent 后置接收提案；角色数量固定，不按日期或平台数量复制 |
