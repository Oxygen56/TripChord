...
下一步不会推倒当前真实流程，也不会先做第二次耗时数天的全窗口实验。实施固定为七次用户可见迭代：先补齐当前单目的地的跨来源整趟比价与等待时间，再依次完成多城市、分地同行、个性化按需 Agent、复杂修改、新交通与活动，以及外部 Agent 接入。冻结来源先快速验证结构；每个新增旅行结构还必须完成一次范围受控的当前来源行程，不能把回放当作实时能力。
