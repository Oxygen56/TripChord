...
它的核心目标很直接：**先找到真正能走、也符合用户要求的行程，再从本次实际查到且能够公平比较的方案中，选择整趟旅行同行总价更低的一份。** 酒店品质、位置、早餐、行李或是否接受中转等明确要求会先参与筛选，不会被低价覆盖。根据不同用户的取舍，在多组各有优势的候选中选择个性化答案，是已经确定的下一步架构方向，不作为 1.0 当前能力宣称。

当前版本是 **1.0.0**。核心规划、偏好记忆和自然语言修改已经形成闭环；它仍是本地开发者版本，不代表签名安装包、生产稳定性、商业采用或全平台覆盖。

项目为什么保留旅行内核、怎样使用 Multi-Agent、为何没有直接迁移 LangGraph，以及 PydanticAI、OpenAI Agents SDK、Google ADK、DBOS、AutoGen、CrewAI 与 Multica 的对比结论，集中记录在[重要架构决策](docs/architecture-decisions.md)中。
...
| 是否引入通用 Agent 框架 | 五种模型 Runtime 已按同一旅行合同完成离线对照；当前生产实现保持不变，PydanticAI 只作为下一次模型层试点候选，完整结论见[重要架构决策](docs/architecture-decisions.md) |
